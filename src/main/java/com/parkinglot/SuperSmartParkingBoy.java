...
public class SuperSmartParkingBoy extends ParkingBoy {
    ...
    public ParkingTicket fetchTicketByCar(Car car) {
        ...
        if (filterParkingLots.size() != 0){
            return filterParkingLots.stream().sorted(Comparator.comparing(ParkingLot::culParkingRates).reversed()).collect(Collectors.toList()).get(0).generateTicketByCar(car);
        }else{
